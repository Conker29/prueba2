...
class CrearCoordinadorScreen extends StatefulWidget {
  const CrearCoordinadorScreen({super.key});
  @override
  State<CrearCoordinadorScreen> createState() => _State();
}

class _State extends State<CrearCoordinadorScreen> {
  ...
  Future<void> _guardar() async {
    ...
    if (ok) {
      mostrarSnack(context,
          'Coordinador creado. Contraseña inicial: $passwordInicial');
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Crear coordinador de brigada')),
          ...
          child: Column(
            children: [
              ...
              const Align(
                  ...
                  child: Text('Asignar a sectores:',
                      style: TextStyle(fontWeight: FontWeight.bold))),
              StreamBuilder<List<SectorModel>>(
                stream: context.read<SectorProvider>().obtenerSectores(),
                  ...
                  return Column(
                    children: snap.data!
                        .map((s) => CheckboxListTile(
                              title: Text(s.nombre),
                              value: _sectoresSel.contains(s.nombre),
                              onChanged: (v) => setState(() {
                                v == true
                                    ? _sectoresSel.add(s.nombre)
                                    : _sectoresSel.remove(s.nombre);
                              }),
                            ))
                        .toList(),
                  );
